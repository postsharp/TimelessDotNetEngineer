...
internal static class TodoListEndpointsExtensions
{
    public static WebApplication MapTodoListEndpoints( this WebApplication app )
    {
        ...
        app.MapGet(
            "/todo/{id}",
            ( TodoService todos, string id, CancellationToken cancellationToken )
                => todos.GetTodoAsync(
        ...
        app.MapPut(
            "/todo/{id}",
            async ( TodoService todos, string id, Todo todo, CancellationToken cancellationToken )
                =>
            {
                if ( await todos.UpdateTodoAsync(
                        int.Parse( id, CultureInfo.InvariantCulture ),
                        todo,
                        cancellationToken ) )
                {
        ...
        app.MapDelete(
            "/todo/{id}",
            async ( TodoService todos, string id, CancellationToken cancellationToken ) =>
            {
                if ( await todos.DeleteTodoAsync(
                        int.Parse( id, CultureInfo.InvariantCulture ),
                        cancellationToken ) )
                {
                    return Results.NoContent();
